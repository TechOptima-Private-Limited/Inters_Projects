...
st.set_page_config(
    layout="wide",
    page_title="AI Powered Credit Analyzer",
    initial_sidebar_state="expanded",
    page_icon="📊"
)

# Custom CSS for enhanced UI (after set_page_config)
st.markdown("""
<style>
    ...
    .report-header { color: #1e88e5; font-size: 2.5em; text-align: center; margin-bottom: 0; }
    .subheader { color: #37474f; font-size: 1.5em; margin-top: 20px; }
    .info-box { background-color: #e3f2fd; padding: 10px; border-radius: 8px; }
    .success-box { background-color: #e8f5e9; padding: 10px; border-radius: 8px; }
    .warning-box { background-color: #fff3e0; padding: 10px; border-radius: 8px; }
    .error-box { background-color: #ffebee; padding: 10px; border-radius: 8px; }
...
class QCAAnalyzer:
    """Qualitative Comparative Analysis for Credit Assessment"""
    ...
    def _fuzzy_calibrate(self, value, threshold_out, threshold_cross, threshold_in, inverse=False):
        if inverse:
            if value <= threshold_in:
                return 1.0
            elif value >= threshold_out:
                return 0.0
            elif value <= threshold_cross:
                return 0.5 + (threshold_cross - value) / (2 * (threshold_cross - threshold_in))
        ...
        if value >= threshold_in:
            ...
            return 0.5 + (value - threshold_cross) / (2 * (threshold_in - threshold_cross))
    ...
    def analyze_sufficiency(self, outcome='GOOD_CREDIT'):
        ...
        return {'single_conditions': results, 'combinations': combination_results}
   
    def generate_qca_report(self, calibrated_data, sufficiency_results):
        ...
        # Company-Specific QCA Insights
        company_name = list(calibrated_data.keys())[0] if calibrated_data else ""
        if company_name and calibrated_data and company_name in calibrated_data:
            report += f"## 🏢 Company-Specific QCA Insights for {company_name}\n\n"
            company_conditions = []
            for condition, score in calibrated_data[company_name].items():
                if score > 0.5:
                    metrics = single_results.get(condition, {'consistency': 0, 'coverage': 0})
                    company_conditions.append(
                        f"- **{condition}** (Membership: {score:.3f}): This condition has a consistency of {metrics['consistency']:.3f}, "
                        f"indicating it is {'' if metrics['consistency'] > 0.8 else 'not '}highly reliable for predicting a good credit profile. "
                        f"It covers {metrics['coverage']:.3f} of good credit cases."
                    )
        
            if company_conditions:
                report += f"{company_name} satisfies the following conditions, contributing to its credit profile:\n"
                report += "\n".join(company_conditions) + "\n\n"
            else:
                report += f"{company_name} does not strongly satisfy any analyzed conditions (membership scores ≤ 0.5).\n\n"
        
            # Analyze combinations
            applicable_combinations = []
            for combo, metrics in sorted(combo_results.items(), key=lambda x: x[1]['consistency'], reverse=True):
                cond1, cond2 = combo.split(' * ')
                if (calibrated_data[company_name].get(cond1, 0) > 0.5 and 
                    calibrated_data[company_name].get(cond2, 0) > 0.5):
                    applicable_combinations.append(
                        f"- **{combo}** (Consistency: {metrics['consistency']:.3f}, Coverage: {metrics['coverage']:.3f}): "
                        f"This combination strongly supports a good credit profile for {company_name}."
                    )
        
            if applicable_combinations:
                report += "Relevant condition combinations for a good credit profile:\n"
                report += "\n".join(applicable_combinations) + "\n\n"
            else:
                report += f"No analyzed combinations are strongly satisfied by {company_name}.\n\n"
        
            # Summary
            outcome = self.truth_table[self.truth_table['Company'] == company_name]['GOOD_CREDIT'].iloc[0] if self.truth_table is not None else 0
            report += f"**Summary**: {company_name} {'has' if outcome == 1 else 'does not have'} a good credit profile based on QCA criteria. "
            report += "The conditions and combinations above highlight the financial strengths contributing to this assessment.\n"
    
        return report

class FinancialHistoryManager:
    """Enhanced class to manage multi-year financial data with QCA"""
    ...
    def detect_companies_in_csv(self, df):
        ...
        if company_columns:
            ...
            for company in unique_companies:
                ...
                if (company_name and company_name.lower() not in ['nan', 'none', '', 'null'] and
                    not company_name.replace('.', '').replace('-', '').isdigit()):
            ...
            for idx, row in df.iterrows():
                for col in df.columns[:3]:
                    ...
                    if (cell_value and cell_value.lower() not in ['nan', 'none', '', 'null'] and
                        not cell_value.replace('.', '').replace('-', '').isdigit() and len(cell_value) > 2):
    ...
    def perform_qca_analysis(self):
        ...
        for company, years_data in self.company_data.items():
            ...
            data = years_data[latest_year]
            score = 0
            if data['current_assets'] / max(data['current_liabilities'], 0.1) > 1.2:
                score += 1
            if data['total_debt'] / max(data['equity'], 0.1) < 1.0:
                score += 1
            if (data['net_profit'] / max(data['revenue'], 0.1)) * 100 > 5:
                score += 1
            if data['ebit'] / max(data['interest_expense'], 0.1) > 2.5:
                score += 1
        ...
        qca_report = self.qca_analyzer.generate_qca_report(calibrated_data, sufficiency_results)
...
st.markdown("<h1 class='report-header'>AI Powered Credit Analyzer</h1>", unsafe_allow_html=True)
                ...
                if st.button("🚀 Auto-Extract All Company Data", type="primary"):
                    ...
                    with st.spinner("Extracting financial data..."):
                        for company in detected_companies:
                            ...
                            if extracted_data:
                                success_count += 1
                                st.markdown(f"<div class='success-box'>✅ {company}: {len(extracted_data)} years of data</div>", unsafe_allow_html=True)
                            else:
                                st.markdown(f"<div class='warning-box'>⚠️ {company}: No financial data found</div>", unsafe_allow_html=True)
                   
                    if success_count > 0:
                        st.balloons()
                        st.markdown(f"<div class='success-box'>🎉 Successfully extracted data for {success_count}/{len(detected_companies)} companies!</div>", unsafe_allow_html=True)
                        st.session_state['csv_extracted'] = True
                        st.rerun()
                    else:
                        st.markdown("<div class='error-box'>❌ No financial data could be extracted. Please check your CSV format.</div>", unsafe_allow_html=True)
            else:
                st.markdown("<div class='warning-box'>⚠️ No companies detected in CSV. Ensure your CSV includes:</div>", unsafe_allow_html=True)
                st.markdown("""
...
# CENTER PANEL
with center:
    tab1, tab2, tab3 = st.tabs(["🏦 Company Analysis", "💬 Analysis Assistant", "✍️ Manual Data Entry"])
    ...
    with tab1:
        ...
        if uploaded_file and csv_extracted and available_companies:
            st.markdown(f"<div class='success-box'>📊 Analysis Ready - {len(available_companies)} companies available</div>", unsafe_allow_html=True)
            ...
            if selected_company:
                ...
                with col1:
                    st.markdown(f"<div class='info-box'>📅 <b>Available Years:</b> {', '.join(available_years)}</div>", unsafe_allow_html=True)
                with col2:
                    st.markdown(f"<div class='info-box'>📈 <b>Data Points:</b> {len(available_years)} years</div>", unsafe_allow_html=True)
...
2. Calculate growth rates and patterns
3. Assess credit risk based on trends
4. Evaluate ratio stability
5. Incorporate QCA findings to identify key conditions for creditworthiness
6. Determine appropriate credit rating
</think>
...
"""
                    ...
                    st.markdown("<h3 class='subheader'>📄 Credit Analysis Report</h3>", unsafe_allow_html=True)
                    with st.spinner("Generating comprehensive analysis..."):
                        response_text, thought = call_optgpt(prompt)
                        st.markdown(response_text, unsafe_allow_html=True)
                        conclusion = extract_conclusion(response_text)
                        if conclusion:
                            st.markdown(f"<h3>{conclusion}</h3>", unsafe_allow_html=True)
                        if thought:
                ...
                if st.button("📈 Show Visualizations"):
                    ...
                    if financial_data:
                        st.markdown("<h3 class='subheader'>📊 Financial Performance Charts</h3>", unsafe_allow_html=True)
                        ...
                        st.markdown("<div class='warning-box'>⚠️ No visualization data available</div>", unsafe_allow_html=True)
       
        else:
            st.markdown("<div class='info-box'>📝 Upload a CSV file in the left panel or use the Manual Data Entry tab to begin analysis.</div>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if available_companies:
            ...
            query_company = st.selectbox("Select Company", available_companies, key="query_company_center")
            ...
            if st.button("🔍 Quick Analysis", key="quick_analysis"):
                trend_data = history_manager.generate_trend_analysis(query_company)
                quick_prompt = ""
                
                if query_type == "QCA Insights":
                    qca_section = ""
                    if len(history_manager.company_data) >= 3:
                        with st.spinner("Performing QCA analysis..."):
                            qca_results = history_manager.perform_qca_analysis()
...
QUALITATIVE COMPARATIVE ANALYSIS (QCA) RESULTS:
{qca_results['qca_report']}
This QCA analysis compares {query_company} with {len(history_manager.company_data)-1} other companies to identify conditions leading to good credit profiles.
"""
                            else:
                                qca_section = f"""
QUALITATIVE COMPARATIVE ANALYSIS (QCA) RESULTS:
...
"""
                    ...
                    quick_prompt = f"""
Provide a concise QCA Insights analysis for {query_company}:
...
Give practical insights for credit decisions based on the QCA results, highlighting key financial conditions and combinations that predict a strong credit profile. If QCA data is insufficient, provide insights based on trends and suggest next steps for obtaining QCA insights.
"""
                else:
                    quick_prompt = f"""
Provide a {query_type} for {query_company}:

{trend_data}

Give practical insights for credit decisions.
"""
                
                with st.spinner("Analyzing..."):
                    response_text, _ = call_optgpt(quick_prompt)
                    st.markdown(f"<div class='success-box'>{response_text}</div>", unsafe_allow_html=True)
        else:
            st.markdown("<div class='info-box'>📝 No companies available. Please upload a CSV in the left panel or enter data manually in the Manual Data Entry tab.</div>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if company_name:
            ...
            if existing_data:
                st.markdown(f"<div class='info-box'>📊 Company exists with {len(existing_data)} years of data</div>", unsafe_allow_html=True)
            ...
            st.markdown("<div class='info-box'>💡 <b>Tip:</b> Enter data for at least 3 companies to enable QCA Analysis.</div>", unsafe_allow_html=True)
            ...
            with col_year2:
                if entry_mode == "Add New Year":
                    ...
                    if existing_data:
                        ...
                        st.markdown("<div class='warning-box'>⚠️ No existing data to edit</div>", unsafe_allow_html=True)
            ...
            with col_btn1:
                if st.button("💾 Save Data", key="save_data"):
                    ...
                    st.markdown(f"<div class='success-box'>✅ Data saved for {company_name} - Year {year}</div>", unsafe_allow_html=True)
                    st.rerun()
...
"""
                        ...
                        st.markdown("<div class='error-box'>❌ Please save financial data for the company before analyzing.</div>", unsafe_allow_html=True)
...
with right:
    ...
    st.markdown("""AI Powered Credit Analyzer empowers financial analysts to evaluate company creditworthiness with Qualitative Comparative Analysis and trend analysis
    """)
    st.markdown("""
**How to Use This Tool:**
- Upload CSV in left panel and extract company data.
- Enter financial data manually in "Manual Data Entry" tab.
- Select company in "Company Analysis" tab for reports or charts.
- Use "Analysis Assistant" tab for quick insights on a company.
""")
